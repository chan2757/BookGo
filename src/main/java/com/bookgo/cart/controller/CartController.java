...
import com.bookgo.cart.service.CartService;
import com.bookgo.dao.ProductMapper;
...
@Slf4j
@Controller
@SessionAttributes({"loginMember", "bookgoCart"})
public class CartController {
	...
	public String addToCart(HttpServletRequest request, HttpSession session, Model model) {
		...
		if(loginMember == null && session.getAttribute("bookgoCart") == null) {
			log.info("비회원용 session 장바구니 생성");
			List<CartDTO> nonmemberCart = new ArrayList<>();
			session.setAttribute("bookgoCart", nonmemberCart);
		...
		for(int i=0; i < optionNoArr.length; i++) {
			...
			if(loginMember == null) {
				...
				List<CartDTO> nonmemberCart = (List<CartDTO>) session.getAttribute("bookgoCart");
				...
				if(memberCart == null) {
					...
					for(int j=0; j < memberCart.size(); j++) {
						...
						if(memberCart.get(j).getOptionNo() == Integer.parseInt(optionNoArr[i])) {
							...
							session.setAttribute("bookgoCart", memberCart);
		...
		log.info("최종 cart items : {}", session.getAttribute("bookgoCart"));
